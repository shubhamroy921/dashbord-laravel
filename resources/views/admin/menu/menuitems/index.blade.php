        ...
        <div class="row">
            <div class="col-12">
                <div class="card p-2 mb-4 mx-4 d-flex flex-row justify-content-between items-center">
                    <h3>Category For : {{ $menu->name }}</h3>
                    <div class="mb-0"><a href="{{ route('admin.menu.index') }}"
                ...
                <div class="card mb-4 mx-4">
                    <div class="card-header pb-0">
                        <div class="d-flex flex-row justify-content-between">
                            ...
                            <div>
                                <form action="{{ route('admin.menuitems.index', $menu) }}" method="GET"
                                    class="d-flex">
                            ...
                            <div>
                                <a href="{{ route('admin.menuitems.create', $menu->id) }}?name={{ urlencode($menu->name) }}"
                                    class="btn bg-gradient-primary btn-sm mb-0" type="button">+&nbsp; Add</a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body px-0 pt-0 pb-2">
                        <div class="table-responsive p-0">
                                    ...
                                    @else
                                        @foreach ($menuitems as $category)
                                            <tr data-id="{{ $category->id }}" class="draggable" draggable="true">
                                                ...
                                                <td class="text-center">
                                                    <a href="{{ route('admin.menuitems.edit', ['menu' => $category->menu_id,'menuitems'=>$category->id]) }}"
                                                        class="me-3" data-bs-toggle="tooltip" data-bs-original-title="Edit Sub Category">
                                                        <i class="fas fa-user-edit text-secondary"></i>
                                                     </a>
                                                   <form action="{{ route('admin.menuitems.destroy', $category->id) }}"
                            ...
                            <div class="d-flex justify-content-center mt-4 mx-3">
                                {{ $menuitems->appends(['search' => $search])->links('vendor.pagination.bootstrap-5') }}
                            </div>
                        </div>
                    </div>
                </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.status-checkbox').forEach(function(checkbox) {
                checkbox.addEventListener('change', function() {
                    ...
                    fetch('{{ route('admin.menuitems.updateStatus') }}', {
                            method: 'POST',
                            headers: {
                        ...
                        .then(data => {
                            if (data.success) {
                                console.log('Items status updated successfully.');
                                toastr.success('Items status updated successfully.');
                            } else {
                                console.error('Failed to update Items status.');
                                toastr.error('Failed to update Items status.');
                            }
                        })
                        .catch(error => {
                            console.error('Error:', error);
                            toastr.error(
                                'An error occurred while updating the Items status.');
        ...
        document.addEventListener('DOMContentLoaded', function() {
            new Sortable(document.querySelector('#sortable-table tbody'), {
                ...
                onEnd: function(evt) {
                    ...
                    fetch('{{ route('admin.menuitems.reorder') }}', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                        ...
                        .then(data => {
                            ...
                            if (data.success) {
                                toastr.success('Items order updated successfully.');
                            } else {
                                toastr.error('Failed to update Items order.');
                            }
                        })
                        .catch(error => {
                            console.error('Error:', error);
                            toastr.error('An error occurred while updating the Items order.');
                        });
                }
            });
        });
